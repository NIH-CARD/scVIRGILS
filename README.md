...
The modules of Scanpy (https://github.com/scverse/scanpy) are utilized heavily to produce an integrated atlas with minimal batch effects. 
...
To launch `python scVIRGILS_gui.py` and enter the required fields.
...
- Metadata file in .csv format, example in `input/example_metadata.csv`. A minimal metadata file should include:
  ...
  - Sample comparison, usually a disease or diagnosis, called 'Primary Diagnosis' in the example, as well for the values of this column (This is not required if using the GUI to launch the pipeline);
...
- Cell-typing table with marker genes, in .csv format, see example in `input/example_marker_genes.csv` (This file is not required if using the GUI to launch the pipeline.)
...
Once set up, if using the command line to launch this pipeline, the complete pipeline can be run by simply typing `bash snakemake.sh` in terminal in an HPC running Slurm or by submitting a sbatch job with `sbatch snakemake.sh`. This is a work in progress and has not been tested on other devices. 
