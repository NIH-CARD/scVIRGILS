...
if [[ ! -f "$REPO_ROOT/convert_h5ad_to_seurat.R" ]]; then
  echo "ERROR: Missing $REPO_ROOT/convert_h5ad_to_seurat.R" >&2
  exit 1
fi
...
singularity exec \
  --bind "$ATLAS_DIR:/atlas" \
  "$REPO_ROOT/envs/r_container.sif" \
  Rscript "$REPO_ROOT/convert_h5ad_to_seurat.R" "/atlas/03_modeled_anndata_rna.h5ad"
